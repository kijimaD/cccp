...
typedef struct Ast {
  ...
  union {
    ...
    struct {
      char *vname; // var name
      int vpos; // var position
      struct Ast *vnext; // var next
    };
    // Binary operator
    struct {
      struct Ast *left;
...
// varsを新しく作成したものに入れ替える。vnextに前のvarsが入っているのでたどることができる
...
/* int,string,symbol,functionを処理する */
Ast *read_prim(void) {
  int c = getc(stdin);
  if (isdigit(c))
...
Ast *read_expr2(int prec) {
  ...
  for (;;) {
    ...
    int c = getc(stdin);
    if (c == EOF) return ast;
    int prec2 = priority(c);    /* 現在の演算子の優先順位を取得する */
    if (prec2 < 0 || prec2 < prec) {         /* 現在の演算子の優先順位が低ければ確定、高ければ評価を右に進める */
      ungetc(c, stdin);
      return ast;
    }
    skip_space();
...
/* 最初読み込むときは優先順位が最も低く読み込む。read_expr2は再帰関数なので、あとは優先順位を判断して再帰する */
Ast *read_expr(void) {
  Ast *r = read_expr2(0);
  if (!r) return NULL;
  skip_space();
